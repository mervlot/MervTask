...
func Validator(conn *pgx.Conn) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		// verify access token
		accessData, err := authentication.VerifyJWTAccess("hello", access_cookie)
		if err != nil {
			fmt.Println("1", err)

			// if access token invalid, try refresh token
			refreshData, eror := authentication.VerifyJWTRefresh("hello", refresh_cookie)
			if eror != nil {
				fmt.Println("2", eror)
			...
			accessData, _ = authentication.VerifyJWTAccess("hello", newAccess)
		}
